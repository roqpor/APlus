...
public interface EatLogDao {
    ...
    @Query("SELECT * FROM EatLog WHERE pillId = :pillId AND isTaken = 1 AND date BETWEEN :dayStart AND :dayEnd")
    List<EatLog> getTakenLogs(int pillId, Date dayStart, Date dayEnd);
